...
- 분산(Var, Variance), 표준편차(SD, Standard Deviation)
    ...
    → 한 집단을 설명하기 위해 대푯값으로 평균을 계산했을 때,
        값들이 평균으로부터 얼마나 벗어나 있는지를 나타내주는 값
...
표본을 뽑고 평균 계산하는 것을 100번 반복 → 각각의 평균 값 100개 획득

평균들의 분포 (표집 분포) → 정규분포에 점점 가까워짐 $N(μ, σ^2/n)$

                                     → 즉, 표집 분포의 평균은 모평균(μ)에 근사함.

→ 표본의 크기(n)이 클수록 정규분포 모양이 중심(Central)에 가까워지는(Limit) 좁은 형태가 됨

→ **각각의 평균** 값 하나하나는 **정규분포에 속한 값**이다. (정규분포로부터 나온 값이기 때문!!)

### 표준 오차 (Standard Error)

![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/336edc9f-be08-4746-ae72-23d96c4f092a/39fc1106-d60f-4625-b771-a4d5e4207407/Untitled.png)

모평균(μ) : 추정하고 싶은 정답

표본평균 : 모평균에 대한 추정치 (예측치)
