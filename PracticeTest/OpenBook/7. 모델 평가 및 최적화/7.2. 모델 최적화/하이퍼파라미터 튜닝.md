...
하이퍼파라미터(Hyperparameter)란 머신러닝 모델의 학습 과정에 사용자가 직접 설정해야 하는 값으로, 모델의 성능에 큰 영향을 미칩니다. 하이퍼파라미터 튜닝은 모델이 최적의 성능을 발휘할 수 있도록 다양한 하이퍼파라미터 조합을 탐색하고 최적의 값을 찾는 과정을 의미합니다. 이 과정은 데이터에 가장 적합한 모델을 구축하기 위한 핵심 단계 중 하나입니다.
...
`GridSearchCV`는 사용자가 지정한 하이퍼파라미터 값들의 모든 조합에 대해 교차 검증을 수행하여 가장 좋은 성능을 내는 조합을 찾는 방법입니다. 격자(Grid)처럼 촘촘하게 모든 경우의 수를 탐색하므로, 탐색 범위 내에서는 최적의 조합을 반드시 찾을 수 있습니다.
...
`RandomizedSearchCV`는 `GridSearchCV`와 달리 모든 조합을 시도하는 대신, 지정된 횟수(`n_iter`)만큼 하이퍼파라미터 값의 조합을 랜덤하게 추출하여 탐색하는 방법입니다. 계산 비용을 절약하면서도 준수한 최적해를 효율적으로 찾을 수 있습니다.
...
- `GridSearchCV`와 동일하게 `best_params_`, `best_score_`, `best_estimator_`, `cv_results_` 속성을 통해 결과를 확인할 수 있습니다.
...
베이지안 최적화는 사전 지식(prior)을 바탕으로 목적 함수(여기서는 모델의 성능)의 형태를 추정하고, 이를 기반으로 다음 탐색할 하이퍼파라미터 지점을 '지능적으로' 결정하는 방법입니다. 이전 탐색 결과를 활용하여 불필요한 탐색은 줄이고, 성능이 좋을 것으로 기대되는 영역을 집중적으로 탐색하여 `GridSearchCV`나 `RandomizedSearchCV`보다 더 적은 횟수로 최적해를 찾는 것을 목표로 합니다.
...
#### 코드 예시 (`scikit-optimize`의 `BayesSearchCV` 사용)
